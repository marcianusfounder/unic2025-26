...
### Exercício 9: Identificação de Modelo a partir de uma Figura (Modelo Incremental)
**Questão:** Uma organização identificou a necessidade de desenvolver um software que melhorasse a gestão da informação. A equipa decidiu utilizar o modelo descrito numa figura que mostra as atividades *Comunicação, Planeamento, Modelagem (análise, projeto), Construção (código, teste) e Desdobramento (entrega, retroação)* repetidas em vários blocos, com a entrega sucessiva do "1.º incremento", "2.º incremento" … "incremento n" ao longo do eixo "Tempo de calendário do projeto".
a) Identifique o modelo. b) Quais são as suas características? c) Na representação gráfica, indique o custo do projeto. d) Neste modelo, a funcionalidade do software é limitada a cada incremento — explique à organização o porquê.

**Resolução Passo-a-Passo:**
- **a) Modelo:** É o **Modelo Incremental** (categoria dos modelos incrementais). Reconhece-se pela repetição das mesmas fases em blocos que entregam incrementos sucessivos de software ao longo do tempo.
- **b) Características:** É **iterativo por natureza**; parte de exigências iniciais bem definidas mas cujo desenvolvimento não é linear; cada incremento entrega uma parte funcional que o cliente revê e refina; é útil quando o pessoal não está totalmente disponível, já que os incrementos iniciais podem ser executados por menos pessoas.
- **c) Custo na representação gráfica:** Ao contrário do modelo Espiral (onde o custo é o **raio** da espiral), no Incremental o esforço/custo distribui-se e **acumula-se ao longo do eixo do tempo (calendário do projeto)**, à medida que cada novo incremento é construído e entregue — o custo total corresponde à soma dos incrementos.
- **d) Porquê a funcionalidade é limitada a cada incremento:** Precisamente para que o software **possa ser revisto pelo cliente** em cada etapa. Ao limitar deliberadamente a funcionalidade de cada entrega, o cliente consegue avaliar, dar retroação e refinar os requisitos, deixando o produto pronto e validado para a fase/incremento seguinte, reduzindo o risco de construir algo errado.

> [!WARNING] Não confundir com o Espiral
> Se a figura mostrasse **anéis/espiral a partir de um centro**, seria o Modelo **Espiral** (raio = custo, produto terminado só no fim da espiral). Se mostra **blocos repetidos com entregas ao longo de uma linha temporal**, é o Modelo **Incremental**.

---

### Exercício 10: Organização da Equipa de Projeto
**Questão:** A forma como uma equipa é organizada influencia a comunicação, a tomada de decisões e a produtividade. a) Indique as formas de organização de equipas em projetos de informática. b) Escolha uma e descreva as suas principais características (vantagens e desvantagens).

**Resolução Passo-a-Passo:**
- **a)** As quatro formas (Dimensão Organizacional) são: **Isomórfica, Especialista, Democrática** e **Com Programador-Líder**.
- **b) Exemplo — Estrutura Isomórfica:** organiza-se segundo os principais módulos do software, ficando cada membro ligado a um módulo específico do início ao fim. *Vantagens:* é organizacionalmente simples, permite executar muitas tarefas em paralelo e as tarefas são claramente definidas e compreendidas. *Desvantagem:* pode gerar dificuldades na integração dos módulos, devido a problemas de comunicação entre os desenvolvedores. (Adequa-se a projetos cujos módulos são relativamente independentes.)
  - *Alternativa — Programador-Líder:* proposta pela **IBM** para projetos de grande complexidade; oposta à democrática; todas as decisões importantes são tomadas por um programador-líder assistido por especialistas de apoio.

---

### Exercício 11: E-Project vs. Projeto Tradicional
**Questão:** Explique o que distingue um e-Project de um projeto de software tradicional e enumere os seus tipos.

**Resolução Passo-a-Passo:**
O **e-Project** é um projeto de natureza informática que envolve a criação ou modificação de código-fonte **desdobrado via Internet**. Distingue-se do projeto tradicional por: **frequência de libertação menor** (dias ou poucos meses, em vez de meses/anos); **alcance do produto entregue menor** (ex.: alterar uma aplicação ou corrigir um bug); e **processo iterativo** em vez de sequencial. Existem **três tipos**:
1. **Nova Construção:** implementação inicial em poucos meses (ex.: e-Shop, B2B); passa pelas fases de descobrimento, projeção, desenvolvimento e desdobramento.
2. **Remodelamento:** alterações (agregar/eliminar/modificar) mantendo o serviço online; processo acelerado com poucas fases; dura semanas.
3. **Manutenção:** ocorre em tempo real (eliminação de bugs, melhorias menores, mudanças de conteúdo); dura poucos dias; usa "páginas de manutenção" registadas em base de dados para acompanhamento (data, versão, esforço estimado/real, urgência, confirmação de qualidade).
